...
import io.greenglass.hydroponics.application.models.implementation.ImplementationVariableMetricModel.Graph.VARIABLE_METRIC
...
import io.greenglass.hydroponics.application.models.installation.PhysicalNodeModel
...
class GraphRepository(
    ...
    fun plantExists(plantId: String): Boolean {
        logger.debug { "plantExists() - $plantId" }
        neo4jService.db.beginTx().use { tx ->
    ...
    fun findOneInstallationNodes(installationId:  String) :  Result<List<PhysicalNode>> {
        logger.debug { "findOneInstallation() $installationId" }
        try {
            neo4jService.db.beginTx().use { tx ->
                val instNode = checkExists(tx.findNode(installationLabel, "installationId", installationId))
                // Get the EoN nodes
                return Result.Success(instNode.getRelationships(OUTGOING, PHYSICAL_NODE)
                    .map { r -> Pair(PhysicalNodeModel(r.endNode).nodeId, r.endNode)}
                    .map { p1 -> Pair(p1.first, p1.second.getSingleRelationship(IMPLEMENTATION_NODE, OUTGOING).endNode)}
                    .map { p2 -> Pair(p2.first, p2.second.getSingleRelationship(NodeType.NODE_TYPE, OUTGOING).endNode)}
                    .map { p3 -> PhysicalNode(p3.first, NodeType(p3.second)) }
                )
            }
        } catch (ex: NotFoundException) {
            return Result.Failure(OBJECT_NOT_FOUND, installationId)
